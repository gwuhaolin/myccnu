...
public class ManageCET {
	...
	public static Cet46Entity get(String XH) {
		...
		try {
			...
			Object o= session.createQuery("from MainEntity as main where main.studentId=?").setString(0, XH).uniqueResult();
...
//		System.out.println(getRemain("2010210817"));
//		System.out.println(getRemain("2010210818"));
//		System.out.println(getRemain("2010210819"));
//		System.out.println(getRemain("2010210820"));
//		System.out.println(get_KH("420050132115308","吴浩麟"));
		String grade="6",date="201312";
		Session session=HibernateUtil.getSession();
		Query query= session.createQuery("from MainEntity where studentId>'2011210000' and studentId<'2012210000'");
		List<MainEntity> all=query.list();
		HibernateUtil.closeSession(session);
		for (MainEntity one:all){
			Cet46Entity re=get(grade,one.getIdNumber(),date);
			if (re!=null){
				re.setDate(date);
				re.setXh(one.getStudentId());
				HibernateUtil.addEntity(re);
